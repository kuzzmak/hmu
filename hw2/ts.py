...
from common import read_file, Player, POSITION, Solution
...
class TS:
    ...
    def run(self) -> Solution:
        current_solution = greedy(self._players, self._budget)
        while not current_solution.valid():
            current_solution = greedy(self._players, self._budget)
...
if __name__ == '__main__':
    instance_path = r'C:\Users\tonkec\Documents\hmu\hw2\instance1.txt'
    players = read_file(instance_path)
    budget = 100
    ts = TS(players, budget)
    solution = ts.run()
