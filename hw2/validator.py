...
class Validator:
    ...
    def __init__(self, instance_path: str, solution_path: str):
        self._instance_path = instance_path
        self._solution_path = solution_path

    def _read(self):
        with open(self._instance_path, "r") as instance:
            reader = csv.reader(instance)
            data = list(reader)
        with open(self._solution_path) as solution:
            team = solution.readline().rstrip("\n").split(",")
            subs = solution.readline().rstrip("\n").split(",")
        return data, team, subs

    def validate(self) -> Tuple[float, float]:
        data, team, subs = self._read()
